...
class HandLandmarkerManager {
    ...
    private static instance : HandLandmarkerManager = new HandLandmarkerManager();

    // 해당 클래스가 지정되어야지만 results를 가져올 수 있음
    private results !: HandLandmarkerResult

    // HandLandmarker 인스턴스 handLandmarker
    ...
    static getInstance() : HandLandmarkerManager {
    ...
    initializeModel = async () => {
        ...
        // HandLandmarker 인스턴스인 handLandmarker 옵션 설정
    ...
    detectLandmarks = (videoElement:HTMLVideoElement, time: number) => {
        // 만약 HandLandmarker의 인스턴스인 handLandmarker가 비어있으면 함수 종료
        if (!this.handLandmarker) return

        // results 변수에 비디오tag에서 가져온 프레임에서의 landmarks 결과값 저장
        const results = this.handLandmarker.detectForVideo(videoElement, time)
...
export default HandLandmarkerManager